      ...
      if (mapEntry && mapEntry.length) {
        ...
        if (mapEntry.length >= 2) {
          ...
          ret.push(
            `<source srcset="/${
              mapEntry[0].path
            }" type="image/${mapEntry[0].path.split(".").pop()}">`
          );
        }
        ret.push(
          `<img src="${"/" + mapEntry[mapEntry.length - 1].path}" alt="${alt ||
            `${slug}’s ${datasource} avatar`}" loading="lazy" class="avatar">`
        );
        if (mapEntry.length >= 2) {
          ret.push("</picture>");
        }
...
module.exports = function(eleventyConfig) {
  ...
  eleventyConfig.addPairedShortcode("callout", function(
    content,
    level = "warn",
    format = "html"
  ) {
  ...
  eleventyConfig.addShortcode("codetitle", function(
    title,
    heading = "Filename"
  ) {
  ...
  eleventyConfig.addShortcode("templatelangs", function(
    languages,
    page,
    whitelist,
    anchor,
    ...
    return `<${parentTag} class="inlinelist">${languages
      .filter(lang => (!whitelist ? true : whitelist.indexOf(lang.ext) > -1))
      .map(
        lang =>
          `<${childTag} class="inlinelist-item${
            page.url == lang.url ? " active" : ""
          }"><a href="${lang.url}${anchor || ""}">${lang.name} <code>*.${
            lang.ext
          }</code></a></${childTag}>`
      )
      .join(" ")}</${parentTag}>`;
  ...
  eleventyConfig.addShortcode("latestVersion", function(
    versions,
    config,
    prefix = "v"
  ) {
  ...
  eleventyConfig.addShortcode("testimonial", function(testimonial) {
    ...
    }${testimonial.text}${!testimonial.indirect ? `” <span class="bio-source">—${shortcodes.link(testimonial.source, shortcodes.avatarlocalcache("twitter", testimonial.twitter, `${testimonial.name}’s Twitter Photo`) + testimonial.name)}` : ``}</span></p></blockquote>`;
  });
  ...
  eleventyConfig.addTransform("remove-aria-hidden-markdown-anchor", function(
    content,
    outputPath
  ) {
    if (outputPath.endsWith(".html")) {
      return content.replace(
        / aria\-hidden\=\"true\"\>\#\<\/a\>/g,
        ` title="Direct link to this heading">#</a>`
      );
    }
