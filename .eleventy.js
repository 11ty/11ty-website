...
\`\`\``);
	...
	// Start of Eleventy Serverless plugin!
	...
	if(!process.env.ELEVENTY_SERVERLESS) {
		...
		// 	}
		// 	let filename = `./netlify/functions/serverless/serverless-collections.json`;
		// 	fs.writeFileSync(filename, JSON.stringify(saved, null, 2));
		// 	console.log( `Writing ${filename}.` );
		// });

		eleventyConfig.on("eleventy.env", (env) => {
			let modules = getNodeModulesList([env.config]).map(name => `require("${name}");`);
			if(modules.length) {
				let filename = "./netlify/functions/serverless/serverless-config-modules.js";
				fs.writeFileSync(filename, modules.join("\n"));
				console.log( `Eleventy Serverless, writing (×${modules.length}): ${filename}` );
			}
		});
		...
		eleventyConfig.on("eleventy.globalDataFiles", (fileList) => {
			...
			if(modules.length) {
				let filename = "./netlify/functions/serverless/serverless-globaldata-modules.js";
				fs.writeFileSync(filename, modules.join("\n"));
				console.log( `Eleventy Serverless, writing (×${modules.length}): ${filename}` );
			}
		});
		...
		eleventyConfig.on("eleventy.dependencyMap", (templateMap) => {
			let outputMap = {};
			let redirects = ["# These were generated by Eleventy Serverless"];

			for(let entry of templateMap) {
				if(entry.isExternal) {
					outputMap[entry.url] = entry.inputPath;

					redirects.push(`${entry.url} /.netlify/functions/serverless 200!`)
				}
			}

			let mapEntryCount = Object.keys(outputMap).length;
			if(mapEntryCount > 0) {
				let mapFilename = "./netlify/functions/serverless/map.json";
				fs.writeFileSync(mapFilename, JSON.stringify(outputMap, null, 2));
				console.log( `Eleventy Serverless, writing (×${mapEntryCount}): ${mapFilename}` );
			}

			if(redirects.length > 1) {
				let redirectsFilename = "./_site/_redirects";
				fs.writeFileSync(redirectsFilename, redirects.join("\n"));
				console.log( `Eleventy Serverless, writing (×${redirects.length}): ${redirectsFilename}` );
			}
		});
	} // end of Eleventy Serverless plugin
