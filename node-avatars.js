...
const skipUrls = [
  "https://www.gravatar.com/avatar/36386473ee7de091db26bd82f8d18ca8?default=404"
];
  ...
  for (let entry of entries) {
    ...
    if (skipUrls.indexOf(url) > -1) {
      if (existing[sluggedName]) {
        ...
        console.log(
          `Skipped ${sluggedName}, couldn’t find existing record:`,
          existing[sluggedName]
        );
      }
    } else {
...
(async function() {
  // Open Collective
  let supporters = require("./_data/supporters.json").filter(
    entry => entry.role.toLowerCase() === "backer"
  );
  ...
  let testimonials = require("./_data/testimonials.json").map(
    entry => entry.twitter
  );
